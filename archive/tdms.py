...
def find_visual_stimuli(data, th, sampling_rate):
    ...
    starts = find_peaks_in_signal(d_filt, 1, 0.004, above=True )[1:]
    ends = find_peaks_in_signal(d_filt, 1, -0.0052, above=False )
...
#f, ax = plt.subplots()
#ax.plot(photodiode_raw)
#plt.show()

stim_times, filtered = find_visual_stimuli(photodiode_raw, 1, 25000)
print(stim_times)
